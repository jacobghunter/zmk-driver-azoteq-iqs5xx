...
static int iqs5xx_read_reg16(const struct device *dev, uint16_t reg, uint16_t *val) {
    ...
    if (ret < 0) {
        LOG_ERR("read_reg16 failed for reg 0x%04X: %d", reg, ret);
        return ret;
    }
    *val = (buf[0] << 8) | buf[1];
    LOG_DBG("read_reg16 reg=0x%04X val=0x%04X", reg, *val);
    return 0;
}
static int iqs5xx_write_reg16(const struct device *dev, uint16_t reg, uint16_t val) {
    ...
    if (ret < 0) {
        LOG_ERR("write_reg16 failed for reg 0x%04X val=0x%04X: %d", reg, val, ret);
    } else {
        LOG_DBG("write_reg16 reg=0x%04X val=0x%04X", reg, val);
    }
    return ret;
}
static int iqs5xx_read_reg8(const struct device *dev, uint16_t reg, uint8_t *val) {
    const struct iqs5xx_config *config = dev->config;
    uint8_t reg_buf[2] = {reg >> 8, reg & 0xFF};
    int ret = i2c_write_read_dt(&config->i2c, reg_buf, sizeof(reg_buf), val, 1);
    if (ret < 0) {
        LOG_ERR("read_reg8 failed for reg 0x%04X: %d", reg, ret);
    } else {
        LOG_DBG("read_reg8 reg=0x%04X val=0x%02X", reg, *val);
    }
    return ret;
}
static int iqs5xx_write_reg8(const struct device *dev, uint16_t reg, uint8_t val) {
    ...
    int ret = i2c_write_dt(&config->i2c, buf, sizeof(buf));
    if (ret < 0) {
        LOG_ERR("write_reg8 failed for reg 0x%04X val=0x%02X: %d", reg, val, ret);
    } else {
        LOG_DBG("write_reg8 reg=0x%04X val=0x%02X", reg, val);
    }
    return ret;
}
static int iqs5xx_end_comm_window(const struct device *dev) {
    ...
    int ret = i2c_write_dt(&config->i2c, buf, sizeof(buf));
    if (ret < 0) {
        LOG_ERR("end_comm_window failed: %d", ret);
    } else {
        LOG_DBG("end_comm_window ok");
    }
    return ret;
}
...
static int iqs5xx_setup_device(const struct device *dev) {
    ...
    if (ret < 0) {
        LOG_ERR("Failed to end comm window during setup: %d", ret);
        return ret;
    }
    LOG_INF("setup_device: complete");
    return 0;
}
